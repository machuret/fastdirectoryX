  ...
  if (!settings) {
    ...
    settings = await prisma.siteSettings.create({
      data: {
        ...
        contactFormDetails: 'Please fill out the form below to get in touch.',
      },
    });
  }
  return settings;
  ...
  if (req.method === 'GET') {
    try {
      ...
      res.status(200).json({
        ...
        contactFormDetails: settings.contactFormDetails,
      });
    } catch (error) {
    ...
    try {
      const { adminEmail, contactFormTitle, contactFormDetails } = req.body;
      ...
      res.status(200).json({
        ...
        settings: {
          ...
          contactFormTitle: updatedSettings.contactFormTitle,
          contactFormDetails: updatedSettings.contactFormDetails,
        }
      });
    } catch (error) {
